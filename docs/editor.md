...
- The interface is rendered from this repository's own `locales/` files. Switch the language in the header, or add `?lang=zh-CN`, `?lang=zh-TW` or `?lang=ja` to the URL.
