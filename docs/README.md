...
GitLocalize turns a GitHub repository into a translation platform. Translators open a static site hosted on GitHub Pages, edit translations in a focused single-string editor, and commit the result to the `i18n` branch. Automation then opens a pull request against `main` for review.
...
The editor ships with an in-memory GitHub stand-in so the interface can be exercised offline: open [`/Editor/?mock=1`](/Editor/?mock=1) and sign in with any token.
