...
var GitHubApiError = class extends Error {
  ...
  detail;
  documentationUrl;
  constructor(status, code, detail, documentationUrl) {
    super(MESSAGES[code]);
...
function classifyError(status, bodyText, apiMessage) {
...
function toApiError(error) {
  ...
  const detail = timedOut ? "the request was aborted after the 15s timeout (a proxy or firewall may be stalling this request)" : error instanceof Error ? error.message : String(error);
...
var GitHubClient = class {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        if (!response.ok) {
          ...
          try {
            const parsed2 = JSON.parse(bodyText);
            apiMessage = parsed2.message ?? "";
            documentationUrl = parsed2.documentation_url;
          } catch {
            apiMessage = bodyText.slice(0, 300);
          }
          if (response.headers.get("x-ratelimit-remaining") === "0") {
            const resetAt = new Date(Number(response.headers.get("x-ratelimit-reset")) * 1e3);
            apiMessage += ` (anonymous quota is 60 requests/hour per IP; signing in raises it to 5,000/hour. Quota resets at ${resetAt.toLocaleTimeString()})`;
          }
        ...
        if (response.status === 204 || options.allowEmpty) return void 0;
  ...
  captureRateLimit(response) {
    ...
    if (Number.isFinite(limit) && Number.isFinite(remaining) && Number.isFinite(reset)) {
      this.lastRateLimit = { limit, remaining, resetAt: reset * 1e3 };
    }
  }
};
function createClient(token2, options) {
  return new GitHubClient(token2, options);
}
...
// src/parser/yaml.ts
var STRINGIFY_OPTIONS = { lineWidth: 0, indent: 2 };
var yamlParser = {
  id: "yaml",
  extensions: [".yml", ".yaml"],
  parse(content) {
    const document = (0, import_yaml.parseDocument)(content, { keepSourceTokens: true });
    if (document.errors.length > 0) {
      throw new Error(`Invalid YAML: ${document.errors[0].message}`);
    }
    return document.toJS() ?? {};
  },
  serialize(document, originalContent) {
    if (originalContent === void 0 || originalContent.trim() === "") {
      return (0, import_yaml.stringify)(document, STRINGIFY_OPTIONS);
    }
    const parsed2 = (0, import_yaml.parseDocument)(originalContent, { keepSourceTokens: true });
    const before = new Map(flatten(parsed2.toJS() ?? {}).map((entry) => [entry.key, entry.value]));
    const after = new Map(flatten(document).map((entry) => [entry.key, entry.value]));
    for (const key of before.keys()) {
      if (!after.has(key)) deletePathFromDocument(parsed2, key);
    }
    for (const entry of flatten(document)) {
      if (before.get(entry.key) === entry.value) continue;
      parsed2.setIn(entry.path, entry.raw);
      if (entry.kind === "multiline") {
        const node = parsed2.getIn(entry.path, true);
        if ((0, import_yaml.isScalar)(node)) node.type = "BLOCK_LITERAL";
      }
    }
    return parsed2.toString(STRINGIFY_OPTIONS);
  }
};
function deletePathFromDocument(document, key) {
  const path = key.split(".").map((segment) => /^\d+$/.test(segment) ? Number(segment) : segment);
  if (getPath(document.toJS(), path) === void 0) return;
  document.deleteIn(path);
}

// src/parser/index.ts
var parsers = [yamlParser, jsonParser];
...
function getParser(formatOrPath) {
  const byId = parsers.find((parser) => parser.id === formatOrPath);
  if (byId) return byId;
  const format = detectFormat(formatOrPath);
  const found = format ? parsers.find((parser) => parser.id === format) : void 0;
  if (!found) {
...
// load-test.ts
var token = process.env.GHTOK;
var client = createClient(token);
var raw = await client.get(
  "/repos/HayashiUme/GitLocalize/contents/locales/app.en.yml?ref=i18n",
  { accept: "application/vnd.github.raw", raw: true }
);
console.log("raw type:", typeof raw, "length:", raw.length);
var parsed = getParser("locales/app.en.yml").parse(raw);
console.log("parsed entries:", parsed.length);
//# sourceMappingURL=load-test.mjs.map
